...
QUnit.module('SQLSource (legacy)', function (hooks) {
    ...
    test('creates a `transformBuilder` upon first access', function (assert) {
        assert.ok(source.transformBuilder, 'transformBuilder has been instantiated');
        // FIXME
        // assert.strictEqual(
    ...
    test('#update sets data and #query retrieves it', async function (assert) {
        ...
        assert.deepEqual(await source.query((q) => q.findRecord({ type: 'planet', id: '1' })), earth, 'objects strictly match');
    });
    QUnit.skip('#upgrade upgrades the cache to include new models introduced in a schema', async function (assert) {
        ...
        let models = utils_1.clone(schema.models);
        ...
        assert.deepEqual(await source.query((q) => q.findRecord({ type: 'person', id: '1' })), person, 'records match');
        assert.deepEqual(await source.query((q) => q.findRelatedRecord(person, 'planet')), { type: 'planet', id: 'earth' }, 'relationship exists');
    });
    ...
    QUnit.skip('#update tracks refs and clears them from hasOne relationships when a referenced record is removed', async function (assert) {
        ...
        assert.deepEqual((_a = (await source.query((q) => q.findRecord({ type: 'moon', id: 'm1' }))).relationships) === null || _a === void 0 ? void 0 : _a.planet.data, { type: 'planet', id: 'p1' }, 'Jupiter has been assigned to Io');
        assert.deepEqual((_b = (await source.query((q) => q.findRecord({ type: 'moon', id: 'm2' }))).relationships) === null || _b === void 0 ? void 0 : _b.planet.data, { type: 'planet', id: 'p1' }, 'Jupiter has been assigned to Europa');
        await source.update((t) => t.removeRecord(jupiter));
        assert.equal(await source.query((q) => q.findRecord({ type: 'planet', id: 'p1' })), undefined, 'Jupiter is GONE');
        assert.equal((_c = (await source.query((q) => q.findRecord({ type: 'moon', id: 'm1' }))).relationships) === null || _c === void 0 ? void 0 : _c.planet.data, undefined, 'Jupiter has been cleared from Io');
        assert.equal((_d = (await source.query((q) => q.findRecord({ type: 'moon', id: 'm2' }))).relationships) === null || _d === void 0 ? void 0 : _d.planet.data, undefined, 'Jupiter has been cleared from Europa');
    });
    QUnit.skip('#update tracks refs and clears them from hasMany relationships when a referenced record is removed', async function (assert) {
        ...
        // );
        assert.ok(records_1.recordsIncludeAll(await source.query((q) => q.findRelatedRecords(jupiter, 'moons')), [io, europa]), 'Jupiter has been assigned to Io and Europa');
        await source.update((t) => t.removeRecord(io));
        assert.equal(await source.query((q) => q.findRecord({ type: 'moon', id: 'm1' })), null, 'Io is GONE');
        await source.update((t) => t.removeRecord(europa));
        assert.equal(await source.query((q) => q.findRecord({ type: 'moon', id: 'm2' })), null, 'Europa is GONE');
        assert.deepEqual(await source.query((q) => q.findRelatedRecords({ type: 'planet', id: 'p1' }, 'moons')), [], 'Europa and Io have been cleared from Jupiter');
    });
    test('#update adds link to hasMany', async function (assert) {
        ...
        assert.deepEqual(await source.query((q) => q.findRelatedRecords({ type: 'planet', id: 'p1' }, 'moons')), [
            {
                type: 'moon',
                id: 'm1',
                relationships: { planet: { data: { type: 'planet', id: 'p1' } } },
    ...
    QUnit.skip("#update does not remove hasMany relationship if record doesn't exist", async function (assert) {
        ...
        await source.update((t) => t.removeFromRelatedRecords({ type: 'planet', id: 'p1' }, 'moons', {
            type: 'moon',
            id: 'moon1',
        }));
        assert.equal(await source.query((q) => q.findRecord({ type: 'planet', id: 'p1' })), undefined, 'planet does not exist');
    });
    QUnit.skip("#update adds hasOne if record doesn't exist", async function (assert) {
        assert.expect(2);
        const tb = source.transformBuilder;
        const replacePlanet = tb.replaceRelatedRecord({ type: 'moon', id: 'moon1' }, 'planet', { type: 'planet', id: 'p1' });
        const addToMoons = tb.addToRelatedRecords({ type: 'planet', id: 'p1' }, 'moons', { type: 'moon', id: 'moon1' });
        let order = 0;
    ...
    QUnit.skip('#update does not add link to hasMany if link already exists', async function (assert) {
        ...
        await source.update((t) => t.addToRelatedRecords(jupiter, 'moons', { type: 'moon', id: 'm1' }));
    ...
    QUnit.skip("#update does not remove relationship from hasMany if relationship doesn't exist", async function (assert) {
        ...
        await source.update((t) => t.removeFromRelatedRecords(jupiter, 'moons', { type: 'moon', id: 'm1' }));
        assert.ok(true, 'patch completed');
    });
    test('#update can add and remove to has-many relationship', async function (assert) {
        ...
        await source.update((t) => t.addToRelatedRecords(jupiter, 'moons', { type: 'moon', id: 'callisto' }));
        assert.ok(records_1.recordsInclude(await source.query((q) => q.findRelatedRecords(jupiter, 'moons')), callisto), 'moon added');
        await source.update((t) => t.removeFromRelatedRecords(jupiter, 'moons', {
            type: 'moon',
            id: 'callisto',
        }));
        assert.notOk(records_1.recordsInclude(await source.query((q) => q.findRelatedRecords(jupiter, 'moons')), callisto), 'moon removed');
    });
    test('#update can add and clear has-one relationship', async function (assert) {
        ...
        const callisto = { id: 'callisto', type: 'moon' };
        await source.update((t) => t.addRecord(callisto));
        await source.update((t) => t.replaceRelatedRecord(callisto, 'planet', {
            type: 'planet',
            id: 'jupiter',
        }));
        assert.ok(records_1.equalRecordIdentities(await source.query((q) => q.findRelatedRecord(callisto, 'planet')), jupiter), 'relationship added');
        await source.update((t) => t.replaceRelatedRecord(callisto, 'planet', null));
        assert.notOk(records_1.equalRecordIdentities(await source.query((q) => q.findRelatedRecord(callisto, 'planet')), jupiter), 'relationship cleared');
    });
    QUnit.skip('does not replace hasOne if relationship already exists', async function (assert) {
        ...
        await source.update((t) => t.replaceRelatedRecord(europa, 'planet', { type: 'planet', id: 'p1' }));
        assert.ok(true, 'patch completed');
    });
    ...
    QUnit.skip('#update removing model with a bi-directional hasOne', async function (assert) {
        var _a, _b, _c;
        assert.expect(5);
        const hasOneSchema = new records_1.RecordSchema({
            models: {
                one: {
                    relationships: {
        ...
        assert.deepEqual((_a = one.relationships) === null || _a === void 0 ? void 0 : _a.two.data, { type: 'two', id: '2' }, 'one links to two');
        assert.deepEqual((_b = two.relationships) === null || _b === void 0 ? void 0 : _b.one.data, { type: 'one', id: '1' }, 'two links to one');
        source.update((t) => t.removeRecord(two));
        assert.equal((_c = (await source.query((q) => q.findRecord({ type: 'one', id: '1' }))).relationships) === null || _c === void 0 ? void 0 : _c.two.data, null, 'ones link to two got removed');
    });
    test('#update merges records when "replacing" and will not stomp on attributes and relationships that are not replaced', async function (assert) {
        ...
        assert.deepEqual(await source.query((q) => q.findRecord({ type: 'planet', id: '1' })), {
            type: 'planet',
            id: '1',
    ...
    QUnit.skip('#update can replace related records but only if they are different', async function (assert) {
        ...
        assert.deepEqual(result, [], 'nothing has changed so there are no inverse ops');
    ...
    test('#update merges records when "replacing" and _will_ replace specified attributes and relationships', async function (assert) {
        ...
        assert.deepEqual(await source.query((q) => q.findRelatedRecords({ type: 'planet', id: '1' }, 'moons')), [
            {
                type: 'moon',
                id: 'm2',
                relationships: {
    ...
    test('#query can retrieve an individual record with `record`', async function (assert) {
        ...
        await source.update((t) => [t.addRecord(jupiter)]);
        assert.deepEqual(await source.query((q) => q.findRecord({ type: 'planet', id: 'jupiter' })), jupiter);
    });
    test('#query can perform a simple attribute filter by value equality', async function (assert) {
        ...
        assert.deepEqual(await source.query((q) => q.findRecords('planet').filter({ attribute: 'name', value: 'Jupiter' })), [jupiter]);
    });
    test('#query can perform a simple attribute filter by value comparison (gt, lt, gte & lte)', async function (assert) {
        ...
        assert.deepEqual(await source.query((q) => {
    ...
    QUnit.skip('#query can perform relatedRecords filters with operators `equal`, `all`, `some` and `none`', async function (assert) {
        let jupiter = {
            type: 'planet',
            id: 'jupiter',
            attributes: {
        ...
        assert.deepEqual(await source.query((q) => q
            .findRecords('planet')
            .filter({ relation: 'moons', records: [theMoon], op: 'equal' })), [earth]);
        assert.deepEqual(await source.query((q) => q
            .findRecords('planet')
            .filter({ relation: 'moons', records: [phobos], op: 'equal' })), []);
        assert.deepEqual(await source.query((q) => q
            .findRecords('planet')
            .filter({ relation: 'moons', records: [phobos], op: 'all' })), [mars]);
        assert.deepEqual(await source.query((q) => q.findRecords('planet').filter({
            relation: 'moons',
        ...
        assert.deepEqual(await source.query((q) => q
            .findRecords('planet')
            .filter({ relation: 'moons', records: [ganymede], op: 'none' })), [earth, mars, mercury]);
    });
    QUnit.skip('#query can perform relatedRecord filters', async function (assert) {
        let jupiter = {
            type: 'planet',
            id: 'jupiter',
            attributes: {
        ...
        assert.deepEqual(await source.query((q) => q.findRecords('moon').filter({ relation: 'planet', record: earth })), [theMoon]);
        assert.deepEqual(await source.query((q) => q.findRecords('moon').filter({ relation: 'planet', record: jupiter })), [europa, ganymede, callisto]);
        assert.deepEqual(await source.query((q) => q.findRecords('moon').filter({ relation: 'planet', record: mercury })), []);
        assert.deepEqual(await source.query((q) => q
            .findRecords('moon')
            .filter({ relation: 'planet', record: [earth, mars] })), [theMoon, phobos, deimos]);
    });
    test('#query can perform a complex attribute filter by value', async function (assert) {
        ...
        assert.deepEqual(await source.query((q) => q
            .findRecords('planet')
            .filter({ attribute: 'atmosphere', value: true }, { attribute: 'classification', value: 'terrestrial' })), [earth, venus]);
    });
    ...
    test('#query can sort by an attribute', async function (assert) {
        ...
        assert.deepEqual(await source.query((q) => q.findRecords('planet').sort('name')), [earth, jupiter, mercury, venus]);
    });
    QUnit.skip('#query can sort by an attribute, even when a particular record has none', async function (assert) {
        let jupiter = { type: 'planet', id: 'jupiter' };
        let earth = {
            type: 'planet',
            id: 'earth',
        ...
        assert.deepEqual(await source.query((q) => q.findRecords('planet').sort('name')), [earth, mercury, venus, jupiter]);
    });
    ...
    test('#query can sort by an attribute in descending order', async function (assert) {
        ...
        assert.deepEqual(await source.query((q) => q.findRecords('planet').sort('-name')), [venus, mercury, jupiter, earth]);
    });
    test('#query can sort by according to multiple criteria', async function (assert) {
        ...
        assert.deepEqual(await source.query((q) => q.findRecords('planet').sort('classification', 'name')), [jupiter, earth, mercury, venus]);
    });
    ...
    test('#query - findRecords - records by identity', async function (assert) {
        ...
        let records = await source.query((q) => q.findRecords([earth, io, { type: 'moon', id: 'FAKE' }]));
    ...
    test('#query - page - can paginate records by offset and limit', async function (assert) {
        ...
        assert.deepEqual(await source.query((q) => q.findRecords('planet').sort('name')), [earth, jupiter, mars, venus]);
        assert.deepEqual(await source.query((q) => q.findRecords('planet').sort('name').page({ limit: 3 })), [earth, jupiter, mars]);
        assert.deepEqual(await source.query((q) => q.findRecords('planet').sort('name').page({ offset: 1, limit: 2 })), [jupiter, mars]);
    });
    test('#query - findRelatedRecords', async function (assert) {
        ...
        await source.update((t) => [t.addRecord(jupiter), t.addRecord(callisto)]);
        assert.deepEqual(await source.query((q) => q.findRelatedRecords({ type: 'planet', id: 'jupiter' }, 'moons')), [callisto]);
    });
    test('#query - findRelatedRecord', async function (assert) {
        ...
        await source.update((t) => [t.addRecord(jupiter), t.addRecord(callisto)]);
        assert.deepEqual(await source.query((q) => q.findRelatedRecord({ type: 'moon', id: 'callisto' }, 'planet')), jupiter);
    });
});
//# sourceMappingURL=sql-source-legacy-test.js.map